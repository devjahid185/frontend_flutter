...
class _MyEducationScreenState extends State<MyEducationScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await _api.get('/education/my');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const ModernAppBar(title: 'আমার প্রতিষ্ঠান', subtitle: 'আপনার যুক্ত করা তালিকা'),
        ...
        child: ListView(
          ...
          children: [
            if (_loading)
              ...
              const Padding(
                padding: EdgeInsets.only(top: 30),
                child: Center(child: Text('আপনার কোনো প্রতিষ্ঠান নেই')),
              )
            else
              ..._items.map((institute) {
                final name = (institute['name'] ?? 'প্রতিষ্ঠান').toString();
                final district = (institute['district'] ?? '').toString();
                return Card(
                  child: ListTile(
                    ...
                    onTap: () => Navigator.of(context).push(
                      MaterialPageRoute(builder: (_) => EducationFormScreen(initial: institute)),
                    ),
                  ),
                );
              }),
