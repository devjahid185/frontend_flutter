...
class EducationFormScreen extends StatefulWidget {
  const EducationFormScreen({super.key, this.initial});
...
class _EducationFormScreenState extends State<EducationFormScreen> {
  ...
  final TextEditingController _eiin = TextEditingController();
  ...
  final TextEditingController _levels = TextEditingController();
  final TextEditingController _mediums = TextEditingController();
  ...
  void _applyInitial() {
    ...
    _eiin.text = (data['eiin'] ?? '').toString();
    _board.text = (data['board'] ?? '').toString();
      ...
      if (_selectedImage != null && instituteId > 0) {
        ...
        await _api.postMultipart(
          ...
          fields: {
            ...
            'target_id': instituteId.toString(),
            'set_primary': 'true',
          },
          files: {
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            _textField(_eiin, 'EIIN (ঐচ্ছিক)'),
            const SizedBox(height: 10),
            _textField(_board, 'বোর্ড (ঐচ্ছিক)'),
            const SizedBox(height: 14),
            ...
            const SizedBox(height: 14),
            _sectionTitle('অতিরিক্ত'),
            _textField(_openingHours, 'খোলার সময়'),
            const SizedBox(height: 10),
            _textField(_levels, 'লেভেল (কমা দিয়ে লিখুন)', maxLines: 2),
            const SizedBox(height: 10),
            _textField(_mediums, 'মাধ্যম (কমা দিয়ে লিখুন)', maxLines: 2),
            const SizedBox(height: 10),
