...
import 'teacher_request_form_screen.dart';
import 'teacher_request_details_screen.dart';

class TeacherRequestListScreen extends StatefulWidget {
  const TeacherRequestListScreen({super.key});

  @override
  State<TeacherRequestListScreen> createState() => _TeacherRequestListScreenState();
}

class _TeacherRequestListScreenState extends State<TeacherRequestListScreen> {
  ...
  Future<void> _load({bool reset = true}) async {
    ...
    try {
      final res = await _api.get('/teacher-requests', query: {
          ...
          children: [
            FilledButton.icon(
              onPressed: () => Navigator.of(context).push(
                MaterialPageRoute(builder: (_) => const TeacherRequestFormScreen()),
              ),
              icon: const Icon(Icons.add),
              label: const Text('নতুন টিউশন রিকোয়েস্ট'),
            ),
            ...
            AnimatedSwitcher(
              ...
              child: _showFilters
                  ? Column(
                      key: const ValueKey('teacher_request_filters'),
                      children: [
  ...
  Widget _requestCard(BuildContext context, Map<String, dynamic> req, ColorScheme scheme) {
    final title = (req['title'] ?? 'টিউশন রিকোয়েস্ট').toString();
    ...
    return InkWell(
      onTap: id > 0
          ? () => Navigator.of(context).push(
                MaterialPageRoute(builder: (_) => TeacherRequestDetailsScreen(requestId: id)),
              )
          : null,
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            if (budget.isNotEmpty) Text('৳ $budget', style: TextStyle(color: scheme.primary, fontWeight: FontWeight.w700)),
          ],
        ),
      ),
    );
