...
class _HospitalDetailsScreenState extends State<HospitalDetailsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await _api.get('/hospitals/${widget.hospitalId}');
      if (res is Map<String, dynamic>) _hospital = res;
      await _loadReviews();
  ...
  Future<void> _loadReviews() async {
    try {
      final res = await _api.get('/reviews/hospital/${widget.hospitalId}');
  ...
  Widget _header(ColorScheme scheme) {
    final name = (_hospital?['name'] ?? '').toString();
  ...
  Widget _info(ColorScheme scheme) {
    ...
    final info = <Map<String, String>>[
      ...
      {'label': 'ইমার্জেন্সি', 'value': getS('emergency_phone')},
      ...
      {'label': 'বেড সংখ্যা', 'value': getS('bed_capacity')},
    ];
  ...
  Widget _actions(ColorScheme scheme) {
    final phone = (_hospital?['phone'] ?? '').toString();
    final emergency = (_hospital?['emergency_phone'] ?? '').toString();
    final website = (_hospital?['website'] ?? '').toString();
    final isOwner = _hospital?['is_owner'] == true;
    return Column(
      children: [
        Row(
          children: [
            ...
            Expanded(
              child: OutlinedButton.icon(
                onPressed: emergency.isNotEmpty ? () => _call(emergency) : null,
