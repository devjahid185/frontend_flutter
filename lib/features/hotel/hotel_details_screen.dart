...
class _HotelDetailsScreenState extends State<HotelDetailsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await _api.get('/hotels/${widget.hotelId}');
      if (res is Map<String, dynamic>) _hotel = res;
      await _loadReviews();
  ...
  Future<void> _loadReviews() async {
    try {
      final res = await _api.get('/reviews/hotel/${widget.hotelId}');
  ...
  Future<void> _openMap() async {
    final lat = _hotel?['lat'];
    final lng = _hotel?['lng'];
    final address = (_hotel?['address'] ?? '').toString();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const ModernAppBar(title: 'হোটেল বিস্তারিত', subtitle: 'সেবা ও যোগাযোগ'),
      body: _loading
  ...
  Widget _header(ColorScheme scheme) {
    final name = (_hotel?['name'] ?? '').toString();
    final type = (_hotel?['type'] ?? '').toString();
    final category = (_hotel?['category_name'] ?? '').toString();
    final imageUrl = (_hotel?['image_url'] ?? '').toString();
  ...
  Widget _info(ColorScheme scheme) {
    String getS(String key, [String fallback = '-']) => (_hotel?[key]?.toString().trim().isNotEmpty ?? false)
        ? _hotel![key].toString()
        : fallback;

    final priceMin = _hotel?['min_price'];
    final priceMax = _hotel?['max_price'];
    String priceText = '-';
    ...
    final info = <Map<String, String>>[
      ...
      {'label': 'চেক-ইন', 'value': getS('check_in')},
      {'label': 'চেক-আউট', 'value': getS('check_out')},
      {'label': 'রুম সংখ্যা', 'value': getS('rooms_total')},
      {'label': 'ভাড়া রেঞ্জ', 'value': priceText},
    ];
  ...
  Widget _amenities(ColorScheme scheme) {
    final amenities = (_hotel?['amenities'] as List?)?.cast<String>() ?? [];
    final services = (_hotel?['services'] as List?)?.cast<String>() ?? [];
      ...
      child: Column(
        ...
        children: [
          ...
          if (amenities.isNotEmpty) ...[
            Wrap(spacing: 8, runSpacing: 8, children: amenities.map((e) => _chip(e, scheme)).toList()),
  ...
  Widget _actions(ColorScheme scheme) {
    final phone = (_hotel?['phone'] ?? '').toString();
    final website = (_hotel?['website'] ?? '').toString();
    final facebook = (_hotel?['facebook'] ?? '').toString();
    final isOwner = _hotel?['is_owner'] == true;
    return Column(
      children: [
        Row(
          children: [
        ...
        if (isOwner)
          OutlinedButton.icon(
            onPressed: () => Navigator.of(context).push(
              MaterialPageRoute(builder: (_) => HotelFormScreen(initial: _hotel)),
            ),
